...
1. 在**你 fork 的那份** GitHub 點進那個檔案
2. 檔案內容右上角有一排小 icon，按 **Copy raw file**（整個檔複製起來）
   —— 也可以按 **Raw** 再全選複製
3. 打開 ChatGPT／Claude／Gemini，**貼上**，前面加一句「我要改成 ＿＿＿＿」
4. 🔴 再補一句「**請回完整檔案，不要只給片段，也不要省略沒改到的部分**」
   —— 不講這句，它會回你「把第 12 行改成…」，你不會知道要貼哪裡
5. 它回完之後**複製整份**，回 GitHub 同一個檔，右上角鉛筆 ✏️
...
```
這是我專案裡的 starter-kit/app/api/ai/route.js，內容如下：

（把整個檔案貼上）

我要改成：＿＿＿＿＿＿＿

請回我完整的檔案內容，不要只給片段，也不要省略沒改到的部分。
```
