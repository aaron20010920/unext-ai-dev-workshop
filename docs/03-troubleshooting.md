...
用 Prompt 8 還原 —— 也就是跟它說「退回上一個能動的版本」，它自己會做。

**不要自己去 GitHub 點。** 這件事跟裝環境、部署一樣，講一句話就好。
...
Codex 的額度**依方案、模型、任務大小而變**，不是固定的每日次數
（一個 task = 它完整跑一輪：讀你的專案、改檔、開 PR）。反覆試錯很容易在一個下午用完。

**省著用的方法**：一次交辦一件小事（見下面「一次只改一件事」），
...
3. 複製它給的完整內容
4. 回到你的 GitHub repo → 點進它給你的那個檔案 → 右上角鉛筆 ✏️
